...
Nie dodawaj żadnego wstępu, komentarzy ani podsumowań, wyjście musi być czystym plikiem JSON.
"""
...
W obrębie jednej grupy mogą występować tylko słowa będące tą samą częścią mowy (np. rzeczowniki)
Format wyjściowy:
Grupa: "słowo1", "słowo2", "słowo3"
Grupa: "słowo4", "słowo5", itd.
Nie dodawaj żadnego wstępu, komentarzy ani podsumowań.
"""

full_input = f"{SYSTEM_PROMPT_TXT}\n\nOto lista słów do analizy:\n{words}"
...
try:
    ...
    json_output_path = Path("output") / f"response_{file_name}_{timestamp}.json"
    txt_output_path = Path("output") / f"response_{file_name}_{timestamp}.txt"
    ...
    if start_index != -1 and end_index != 0:
        ...
        try:
            ...
            print(f"Success! Saved JSON to {json_output_path}")

        except Exception as json_error:
            print(f"JSON parse/save failed: {json_error}")

            with open(txt_output_path, "w", encoding="utf-8") as txtfile:
                txtfile.write(raw_text)

            print(f"Saved raw response to TXT: {txt_output_path}")
